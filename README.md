...
```
你："帮我从零构建一个文本统计 skill，目标 90 分"
  ↓
Agent A 从零构建项目（代码+文档+测试）→ Agent B 评审 → 65分
  ↓（自动继续，不等你确认）
Agent A 改进 → Agent B 再评 → 80分
  ↓（自动继续）
Agent A 改进 → Agent B 终评 → 99分 ✅ 达标，结束
```
...
- **从零构建**：一句话需求，Agent 生成完整项目（代码+文档+测试）
...
本 skill 的核心机制是**派发子代理**——主 Agent 将"构建"和"评审"分别委托给两个独立的子 Agent 执行。

| 平台 | 子代理能力 | 状态 |
|------|-----------|------|
| [Hermes Agent](https://github.com/NousResearch/hermes-agent) | `delegate_task` 工具 | ✅ 已测试 |
| CrewAI | Crew + Agent 分工 | 理论兼容 |
| AutoGen | Multi-agent conversation | 理论兼容 |
| LangGraph | Subgraph delegation | 理论兼容 |

> ⚠️ 不支持子代理的平台，可通过**单 Agent 角色切换**实现核心概念，但效果会有折扣。
...
**核心原因：人不会让作者自己给自己的论文打分。** 评审的客观性来自视角分离。
...
│   └── references/
│       ├── implementer-prompts.md      # Agent A prompt 模板
│       ├── reviewer-prompts.md         # Agent B prompt 模板
│       ├── scoring-rubric.md           # 7 维度评分细则
│       ├── feedback-format.md          # feedback.md 格式规范
...
```
脚本单元测试：91 passed / 0 failed / 26 个测试用例
端到端测试：Round 1: 65分 → Round 2: 80分 → Round 3: 99分 ✅ 达标
```
